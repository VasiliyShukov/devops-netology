...
| ------------- | ------------- | ------------- |
| `c`  | a+b  | команда складывает не переменные а текст |
| `d`  | 1+2  | $ обозначает переменную. Соответственно уже подставляются строковые значения переменных a и b |
| `e`  | 3  | $((expression)) Арифметическое расширение. Результат будет выводом не строковых значений, а числовых. |
...
### Ваш скрипт:
Насколько я понял, должно быть вот так. Но он, все равно, не прекращает выполнение. Если, после выполнения скрипта, выполнить $? то выдает 130
